...
- **等级配额** — 新注册用户等级 1（保留 1 条消息 × 1 个月），等级 N 可保留 N 条 × N 个月（最大 99）。等级 0 = 拉黑，禁止注册消息。管理员可在后台调整用户等级（0–99）
...
新注册用户为 **等级 1**。等级 N 表示：最多可保留 **N 条消息**，每条最多保留 **N 个月**（N 最大 99）。注册新消息时，超出配额的最早消息自动删除（仅注册时惰性清理）。**等级 0 = 拉黑**，禁止注册消息。管理员可在后台调整用户等级（0–99）。
